    ...
    const handleSubmitAdd = () => {
        if (isTeamEvent(eventObj)) {
            if (!reg.leader?.name || !reg.leader?.email || !reg.leader?.phone) {
                alert("Please fill team leader name, email and phone.");
                return;
            }
            if (!/^\d{10}$/.test(String(reg.leader.phone))) {
                alert("Leader phone must be 10 digits.");
                return;
            }
            if (reg.leader.altPhone && !/^\d{10}$/.test(String(reg.leader.altPhone))) {
                alert("Alternate phone (if provided) must be 10 digits.");
                return;
            }
            if (Array.isArray(reg.members)) {
                for (const m of reg.members) {
                    if ((m?.name && !m?.email) || (!m?.name && m?.email)) {
                        alert("Each member must have both name and email.");
                        return;
                    }
                }
            }
                            ...
                            <>  
                                ...
                                {mode === 'add' && isTeamEvent(eventObj) && (
                                    <div className="team-form">
                                        <h3>Team Details</h3>
                                        <input type="text" placeholder="Team Leader Name" value={reg.leader?.name || ""} onChange={(e) => handleLeaderChange('name', e.target.value)} />
                                        <input type="email" placeholder="Team Leader Email" value={reg.leader?.email || ""} onChange={(e) => handleLeaderChange('email', e.target.value)} />
                                        <input type="number" placeholder="Team Leader Phone" value={reg.leader?.phone || ""} onChange={(e) => handleLeaderChange('phone', e.target.value)} />
                                        <input type="number" placeholder="Alternate Phone (optional)" value={reg.leader?.altPhone || ""} onChange={(e) => handleLeaderChange('altPhone', e.target.value)} />
                                        <h4>Members (up to {maxAdditional})</h4>
                                        {Array.isArray(reg.members) && reg.members.map((m, idx) => (
                                            <div key={idx} className="team-member-row">
                                                <input type="text" placeholder={`Member ${idx + 1} Name`} value={m.name || ""} onChange={(e) => updateMember(idx, 'name', e.target.value)} />
                                                <input type="email" placeholder={`Member ${idx + 1} Email`} value={m.email || ""} onChange={(e) => updateMember(idx, 'email', e.target.value)} />
                                                <i className="fa-solid fa-xmark" onClick={() => removeMember(idx)}></i>
                                            </div>
                                        ))}
                                        {(reg.members?.length || 0) < maxAdditional && (
                                            <button type="button" className="view-rule-btn" onClick={addMember}>Add Member</button>
                                        )}
                                    </div>
                                )}
                                ...
                                <div className="coordinator-details">
                                    ...
                                    {selectedEvent.includes(getId(eventObj)) ? (
                                        <button className='event-selected-button' onClick={() => selectEvent(getId(eventObj))}>Remove</button>
                                    ) : (
                                        mode === 'add' ? (
